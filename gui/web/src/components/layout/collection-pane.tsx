        ...
        <span key={seg.id} className="flex items-center gap-1">
          ...
          <button
            className={cn(
              ...
              i === segments.length - 1 ? 'text-foreground font-medium cursor-default' : 'cursor-pointer',
            )}
            onClick={() => {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center gap-3 text-center px-6">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Use the Run button to execute this collection</p>
      </div>
    </div>
  );
}
...
function Overview({ name, description, itemCount }: { name: string; description: string; itemCount: number }) {
  return (
    <ScrollArea className="h-full">
      <div className="p-4 space-y-4">
        ...
        <div className="text-xs text-muted-foreground">{itemCount} request{itemCount !== 1 ? 's' : ''}</div>
      </div>
    </ScrollArea>
  );
}
...
export function CollectionPane() {
  ...
  const { collections, findFolderPath, updateCollectionScript, updateItemScript } = useWorkspaceStore();
  ...
  const folderPath = !isCollection && activeTab.folderId
    ? findFolderPath(activeTab.folderId)
    : null;

  const folder = !isCollection && activeTab.folderId
    ? (() => {
        function findFolder(items: import('@/types').CollectionItem[], id: string): import('@/types').FolderItem | null {
  ...
  const collectionSubTabs: CollectionSubTab[] = ['overview', 'authorization', 'variables', 'scripts', 'runs'];
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-background">
        ...
        <div className="flex-1 overflow-hidden">
          <Breadcrumb
            ...
            onFolderClick={(id) => {
              ...
              function findFolderItem(items: import('@/types').CollectionItem[], fid: string): import('@/types').FolderItem | null {
                for (const item of items) {
                  if (!('method' in item)) {
      ...
      <Tabs
        value={subTab}
        onValueChange={(v) => setTabCollectionSubTab(activeTabId, v as CollectionSubTab | FolderSubTab)}
