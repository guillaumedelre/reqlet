...
const BODY = { type: 'none' as const, raw: '', rawContentType: 'application/json' as const, formData: [], urlencoded: [], graphqlQuery: '', graphqlVariables: '' };

function makeTab(overrides: Partial<Tab> & Pick<Tab, 'id' | 'type'>): Tab {
  return {
    title: 'Tab',
    dirty: false,
    request: { method: 'GET', url: '', params: [], headers: [], body: BODY, auth: { type: 'inherit' }, preRequestScript: '', testScript: '' },
    isSending: false,
    response: null,
...
function makeRequest(id: string): RequestItem {
  return { id, name: 'Request', method: 'GET', url: '', params: [], headers: [], body: BODY, auth: { type: 'inherit' }, preRequestScript: '', testScript: '' };
}

function makeFolder(id: string, items: Collection['items'] = []): FolderItem {
  return { id, name: 'Folder', auth: { type: 'inherit' }, preRequestScript: '', testScript: '', items };
}

function makeCollection(id: string, items: Collection['items'] = []): Collection {
  return { id, name: 'Col', description: '', auth: { type: 'none' }, variables: [], preRequestScript: '', testScript: '', items };
}
