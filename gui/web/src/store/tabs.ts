...
import type {
  Tab, RequestState, RequestSubTab, ResponseSubTab,
  RequestItem, Collection, FolderItem, Environment,
  CollectionSubTab, FolderSubTab,
} from '@/types';
  ...
  persist(
    (set, get) => ({
      ...
      reopenLastClosedTab: () => {
        ...
        set({ tabs: [...tabs, restored], activeTabId: restored.id, closedTabs: closedTabs.slice(0, -1) });
      },

      openCollectionTab: (collection: Collection) => {
        const { tabs } = get();
        const existing = tabs.find((t) => t.type === 'collection' && t.collectionId === collection.id);
        if (existing) {
      ...
      closeTabsToRight: (id) => {
        ...
        const nextClosed = [...closedTabs, ...closed].slice(-10);
        const nextActive = kept.some((t) => t.id === activeTabId) ? activeTabId : kept[kept.length - 1].id;
