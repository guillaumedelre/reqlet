...
function env(id = "env-1"): Environment {
  return { id, name: "Dev", variables: [] }
}
function col(id = "col-1"): Collection {
  return {
    id,
    name: "My Col",
    description: "",
...
function folder(id = "fold-1"): FolderItem {
  return {
    id,
    name: "My Folder",
    auth: { type: "none" },
...
describe("updateTabRequest", () => {
  it("updates request fields and marks tab dirty", () => {
    ...
    useTabsStore
      .getState()
      .updateTabRequest(id, (r) => ({ ...r, url: "https://new.example.com", method: "POST" }))
    const tab = useTabsStore.getState().tabs[0]
...
describe("tab update operations do not affect sibling tabs", () => {
  it("updateTabRequest leaves sibling tab unchanged", () => {
    ...
    useTabsStore
      .getState()
      .updateTabRequest(a.id, (r) => ({ ...r, url: "https://changed.example.com" }))
  ...
  it("setTabCollectionSubTab leaves sibling tab unchanged", () => {
    ...
    expect(useTabsStore.getState().tabs.find((t) => t.id === b.id)!.collectionSubTab).toBe(
      "overview",
    )
  })
})
